...
plugins {
    ...
    // id("app.brant.amazonappstorepublisher") WIP NOT WORKING YET!!!
    id("idea")
    id("kotlin-android")
    id("kotlin-parcelize")
...
fun gitCommitHash(): String {
    ...
    return String(outStream.toByteArray()).trim()
}
...
android {
    ...
    defaultConfig {
        ...
        buildConfigField("String", "BACKEND_VERSION", "\"${rootProject.extra["ankidroid_backend_version"]}\"")
    ...
    buildTypes {
        debug {
            ...
            if (project.rootProject.file("local.properties").exists()) {
                ...
                isTestCoverageEnabled = localProperties["enable_coverage"] != "false"
                ...
                if (localProperties["allow_unsafe_migration"] != null) {
                    buildConfigField("Boolean", "ALLOW_UNSAFE_MIGRATION", localProperties["allow_unsafe_migration"] as String)
                }
        ...
        release {
            ...
            splits.abi.isUniversalApk = rootProject.extra["universalApkEnabled"] as Boolean // Build universal APK for release with `-Duniversal-apk=true`
            ...
            if (project.hasProperty("customSuffix")) {
                ...
                applicationIdSuffix = (project.property("customSuffix") as String).replaceFirst("^\\.*".toRegex(), ".")
                resValue("string", "applicationId", "${defaultConfig.applicationId}$applicationIdSuffix")
            } else {
        ...
        if (buildType.name == "release") {
            outputs.all {
                ...
                if (outputFile != null && outputFile.name.endsWith(".apk")) {
                    val abi: FilterData? = filters.find { it.filterType == "ABI" }
                    if (abi != null) { // null for the universal-debug, universal-release variants
                        ...
                        // reason for cast see https://stackoverflow.com/questions/53680668/versioncodeoverride-equivalent-for-gradle-kotlin-dsl
                        (this as com.android.build.gradle.internal.api.ApkVariantOutputImpl).versionCodeOverride =
                            versionCodes[abi.identifier]!! * 100000000 + defaultConfig.versionCode!! // ex:  321200106 = 3 * 100000000 + 21200106
                    }
                }
            }
        }
...
val assertNonzeroAndroidTests = tasks.register("assertNonzeroAndroidTests") {
    doLast {
        ...
        val listOfFiles = folder.listFiles { d, f -> f.matches(".*\\.xml".toRegex()) }
        for (file in listOfFiles) {
            ...
            if (matches.size != 1) {
                throw GradleScriptException("Unable to determine count of tests executed for " + file.name + ". Regex pattern out of date?", null)
            }
            if (!(matches[0].matches(".* tests=\"\\d+\" .*".toRegex())) || matches[0].contains("tests=\"0\"")) {
                throw GradleScriptException("androidTest executed 0 tests for " + file.name + " - Probably a bug with the emulator. Try another image.", null)
            }
        }
    }
}
...
dependencies {
    ...
    implementation("androidx.fragment:fragment-ktx:${rootProject.extra["fragments_version"]}")
    debugImplementation("androidx.fragment:fragment-testing-manifest:${rootProject.extra["fragments_version"]}")
    ...
    if (localProperties["local_backend"] == "true") {
        ...
        implementation("io.github.david-allison-1:anki-android-backend:${rootProject.extra["ankidroid_backend_version"]}")
        testImplementation("io.github.david-allison-1:anki-android-backend-testing:${rootProject.extra["ankidroid_backend_version"]}")
    }

    // May need a resolution strategy for support libs to our versions
    implementation("ch.acra:acra-http:${rootProject.extra["acra_version"]}")
    implementation("ch.acra:acra-dialog:${rootProject.extra["acra_version"]}")
    implementation("ch.acra:acra-toast:${rootProject.extra["acra_version"]}")
    implementation("ch.acra:acra-limiter:${rootProject.extra["acra_version"]}")
    ...
    testImplementation("androidx.test.ext:junit:${rootProject.extra["androidx_test_junit_version"]}")
    testImplementation("org.jetbrains.kotlin:kotlin-reflect:${rootProject.extra["kotlin_version"]}")
    testImplementation("org.jetbrains.kotlin:kotlin-test:${rootProject.extra["kotlin_version"]}")
    testImplementation("org.jetbrains.kotlin:kotlin-test-junit5:${rootProject.extra["kotlin_version"]}")
    ...
    testImplementation("androidx.fragment:fragment-testing:${rootProject.extra["fragments_version"]}")

    // May need a resolution strategy for support libs to our versions
    androidTestImplementation("androidx.test.espresso:espresso-core:${rootProject.extra["espresso_version"]}")
    androidTestImplementation("androidx.test.espresso:espresso-contrib:${rootProject.extra["espresso_version"]}") {
        exclude(module = "protobuf-lite")
    }
    androidTestImplementation("androidx.test:core:${rootProject.extra["androidx_test_version"]}")
    androidTestImplementation("androidx.test.ext:junit:${rootProject.extra["androidx_test_junit_version"]}")
    androidTestImplementation("androidx.test:rules:${rootProject.extra["androidx_test_version"]}")
    androidTestImplementation("org.jetbrains.kotlin:kotlin-test:${rootProject.extra["kotlin_version"]}")
    androidTestImplementation("org.jetbrains.kotlin:kotlin-test-junit:${rootProject.extra["kotlin_version"]}")
    androidTestImplementation("androidx.fragment:fragment-testing:${rootProject.extra["fragments_version"]}")
}
