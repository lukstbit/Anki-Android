...
// TODO CardAnalysisWidgetPreferences needs to be refactored for better access here in tests/injection
...
class CardAnalysisWidgetConfigTest : RobolectricTest() {
    ...
    @Test
    fun `finishes at start if id is INVALID_APPWIDGET_ID`() =
        runTest {
            val activity =
    ...
    fun `configures correctly a new widget`() =
        runTest {
            ...
            onView(withId(R.id.deck_card_name)).check(matches(withText("")))
            // select deck 1
            onView(withText(R.string.select_deck_title))
                .inRoot(isDialog())
                .check(matches(isDisplayed()))
            onView(withText(TEST_DECK_NAME1)).inRoot(isDialog()).perform(click())
            // check if the UI shows the select deck
            onView(withId(R.id.deck_card_name)).check(matches(withText(TEST_DECK_NAME1)))
            // check that the deck is saved in preferences(this is only done on 'done' click)
    ...
    fun `ui is updated based on user deck selection`() =
        runTest {
            ...
            onView(withId(R.id.deck_card_name)).check(matches(withText(TEST_DECK_NAME1)))
            // open deck selection dialog
            onView(withId(R.id.change_btn)).perform(click())
            // select deck 2 and verify that is displayed and selected in ui
            onView(withText(TEST_DECK_NAME2)).inRoot(isDialog()).perform(click())
            onView(withId(R.id.deck_card_name)).check(matches(withText(TEST_DECK_NAME2)))
            // set configuration as done
            onView(withId(R.id.done_btn)).perform(click())
            // check that the correct deck is saved in preferences(this is only done on 'done' click)
    ...
    fun `handles decks selection on configuration change`() =
        runTest {
            ...
            // open deck selection dialog
            onView(withId(R.id.change_btn)).perform(click())
            // select deck 2 and verify that is selected in ui
            onView(withText(TEST_DECK_NAME2)).inRoot(isDialog()).perform(click())
            onView(withId(R.id.deck_card_name)).check(matches(withText(TEST_DECK_NAME2)))
            ...
            // set configuration as done
            onView(withId(R.id.done_btn)).perform(click())
            // check that the last selected deck is saved in preferences(this is only done on 'done' click)
            assertEquals(testDeck2Id, preferences.getSelectedDeckIdFromPreferences(testWidgetId))
        }
