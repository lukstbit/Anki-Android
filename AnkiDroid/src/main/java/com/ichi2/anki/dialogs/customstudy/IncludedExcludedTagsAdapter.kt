...
/**
 * Shows a simple list of tags from which the user can select for his custom study session. For
...
class IncludedExcludedTagsAdapter(
    ...
    init {
        var ta = context.obtainStyledAttributes(intArrayOf(android.R.attr.selectableItemBackground))
        selectableItemBackground = ta.getResourceId(0, 0)
        ta.recycle()
        ta = context.obtainStyledAttributes(intArrayOf(com.google.android.material.R.attr.colorPrimary))
        selectedItemBackground = ta.getColor(0, Color.BLUE)
        ta.recycle()
    }
    ...
    override fun onBindViewHolder(
        ...
        holder.tagView.text = model.name
        val isSelected =
            when (mode) {
